...
- **現在値＋[変更] の確認UI**: 支出区分/立替/カテゴリの現在値をリッチテキストで表示し、変更は [変更] ボタンで行う
...
| 残り予算 | 緑（余裕）/ 赤（残り2万以下） |
| 現在の設定 | `支出区分：…` `立替：…` `カテゴリ：…` の3行。各行の右端に [変更] |
...
| 操作 | Firestoreのstatus | 備考 |
|-------|-------------------|---------------|
| 支出区分 [変更] | shared ⇔ personal | `includeInTotal` を連動。立替は解除 |
| 立替 [変更] | advance_pending ⇔ shared | 立替ありは押下者を `advanceBy` に記録。月末精算に含める |
